...
interface CreateRoleModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCreate: (roleData: NewRoleData) => void;
}
...
function setAllPermissions(module: Module, granted: boolean): Module {
  return {
    ...
    permissions: module.permissions.map(p => ({
      ...p,
      granted,
      children: p.children?.map(c => ({ ...c, granted }))
    }))
  };
}
...
function isModuleFullyGranted(module: Module): boolean {
  return module.permissions.every(p => {
    if (p.children) return p.children.every(c => c.granted);
    return p.granted;
  });
}

function isModulePartiallyGranted(module: Module): boolean {
  const hasAny = module.permissions.some(p => {
    if (p.children) return p.children.some(c => c.granted);
    return p.granted;
  });
...
export function CreateRoleModal({ isOpen, onClose, onCreate }: CreateRoleModalProps) {
  ...
  const handleCreate = () => {
    onCreate({ name, description, modules });
    handleReset();
    onClose();
  };
        ...
        permissions: module.permissions.map(permission => {
          if (permission.key === permissionKey) {
            ...
            if (permission.children) {
              return { ...permission, granted: newGranted, children: permission.children.map(c => ({ ...c, granted: newGranted })) };
            }
      ...
      <div className="space-y-3">
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <label className="text-[10px] font-semibold text-gray-900">Módulos y Permisos</label>
          </div>
