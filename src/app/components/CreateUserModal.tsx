...
import { Search, X, Check, ChevronDown, Shield, Layers } from 'lucide-react';
...
export function CreateUserModal({ isOpen, onClose, onCreate }: CreateUserModalProps) {
  ...
  const [directModules, setDirectModules] = useState<Module[]>([]);
  ...
  const handleCreate = () => {
    onCreate({ firstName, lastName, email, userType, roles: selectedRoles, modules: directModules });
    handleReset(); onClose();
  };
  const handleReset = () => { setFirstName(''); setLastName(''); setEmail(''); setUserType('Usuario'); setSelectedRoles([]); setDirectModules([]); setExpandedModules([]); setRoleSearchQuery(''); };
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Nuevo Usuario" maxWidth="2xl" footer={
      <><ModalButton onClick={handleClose} variant="secondary">Cancelar</ModalButton><ModalButton onClick={handleCreate} variant="primary">Crear Usuario</ModalButton></>
    }>
